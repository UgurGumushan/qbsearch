...
class traht:
    url: str = "https://traht.org"
    name: str = "Traht"
    supported_categories: ClassVar[dict[str, str]] = {"all": "all"}

    pagination_regex: str = r'<div class="paginator_pages">.*?<\/div>'
    ...
    class MyHtmlParser(HTMLParser):
        ...
        def __init__(self, url: str):
            ...
            self.url: str = url
            self.row: SearchResults = self._new_row()

            self.column: int = 0

            self.releaseTableFound: bool = False
            self.insideResultTbody: bool = False
            self.insideRow: bool = False
            self.insideCell: bool = False

            self.shouldGetName: bool = False
            self.shouldParseLink: bool = True
            self.shouldGetSize: bool = False
            self.shouldGetPeers: bool = False
        ...
        def handle_endtag(self, tag: str):
            ...
            if self.insideRow and tag == self.TR:
                result_link = str(self.row.get("link") or self.row.get("desc_link") or "")
