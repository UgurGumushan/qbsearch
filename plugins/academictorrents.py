...
_MIRRORS_RE = re.compile(
    r"<tr><td>Mirrors</td><td>(\d+)\s*complete,\s*(\d+)\s*downloading"
)
_ADDED_DATE_RE = re.compile(r"<tr><td>Added</td><td>([^<]+)</td></tr>")
...
class academictorrents:
    ...
    def _torrent_filter(self, item: ET.Element) -> bool:
        ...
        return not self.filters or any(f in title or f in desc for f in self.filters)
    ...
    def resolve_search_result(self, torrent: ET.Element) -> SearchResults:
        ...
        peer_data = _MIRRORS_RE.search(torrent_desc)
        seeds = -1
        leech = -1
        if peer_data:
        ...
        added_date_data = _ADDED_DATE_RE.search(torrent_desc)
        if added_date_data:
    ...
    def search(self, what: str, _cat: str = "all") -> None:
        self.filters = list(
            dict.fromkeys(f.lower() for f in re.split(r"%20|\s+", str(what)) if f)
        )
        db = self._retrieve_database()
        jobs: list[tuple[ET.Element]] = []
        seen_details: set[str] = set()
        for torrent in db.findall("channel/item"):
            if not self._torrent_filter(torrent):
                continue
            detail_key = (
                torrent.findtext("link")
                or torrent.findtext("infohash")
                or torrent.findtext("title")
                or ""
            )
