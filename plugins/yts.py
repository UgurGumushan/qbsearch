...
def _parse_response(value: object) -> yts_response:
    payload = cast(dict[str, object], value)
    return yts_response(
        status=cast(str, payload.get("status", "")),
        status_message=cast(str, payload.get("status_message", "")),
        data=cast(yts_data, payload.get("data", {})),
    )
...
class yts:
    ...
    url: str = "https://yts.bz/"
    api_url: str = "https://movies-api.accel.li/api/v2/list_movies.json?"
    name: str = "YTS"
    ...
    def search(self, what: str, _cat: str = "all"):
        ...
        try:
            ...
            response_json = cast(dict[str, object], cast(object, json.loads(response_raw)))
            api_result = _parse_response(response_json)
        ...
        self.process_movies(api_result.data.movies or [], search_params, seen_links)
        if api_result.data.limit <= 0:
            return
        total_pages = max(
            1,
            (max(0, api_result.data.movie_count) + api_result.data.limit - 1)
            // api_result.data.limit,
        )
        for page_no in range(2, min(total_pages, MAX_PAGES) + 1):
            try:
                api_result = _parse_response(
                    cast(object, json.loads(retrieve_url(search_url + f"&page={page_no}")))
                )
                self.process_movies(api_result.data.movies or [], search_params, seen_links)
            except Exception as e:
                print(f"Error parsing YTS response: {e}")
    ...
    def process_movies(
        self, movies: list[yts_movie] | None, search_params: dict[str, str], seen_links=None
    ) -> None:
        for movie in movies or []:
            for torrent in movie.torrents or []:
                if seen_links is not None and torrent.url and torrent.url in seen_links:
                    continue
                if (
                ...
                if seen_links is not None and torrent.url:
                    seen_links.add(torrent.url)
