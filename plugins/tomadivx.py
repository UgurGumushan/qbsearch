...
class tomadivx:
    ...
    class MyHtmlParser(HTMLParser):
        ...
        def _claim_detail(self, link: str) -> bool:
            with self.detail_lock:
                if link in self.seen_detail_links or len(self.seen_detail_links) >= MAX_DETAILS:
                    return False
                self.seen_detail_links.add(link)
                return True
        ...
        def handle_starttag(self, tag: str, attrs: list[tuple[str, str | None]]) -> None:
            ...
            if self.insideResultSpan and tag == self.A:
                ...
                if magnet_match is None or size_match is None:
                    ...
                    return
                try:
                    self.row["link"] = "https:" + magnet_match.group(1)
                    size_el = re.sub(r"<b.+?>Tamaño:</b>", "", size_match.group())
                    root = ET.fromstring(size_el)
                    self.row["size"] = (root.text or "").replace(",", ".")
                except (ET.ParseError, IndexError, TypeError, ValueError):
                    self.skip_result = True
                self.seeds = -1
                self.leech = -1
                return
        ...
        def handle_endtag(self, tag: str) -> None:
            ...
            if (
                self.insideResult
                and tag == self.P
                and not self.skip_result
                and "link" in self.row
                and "size" in self.row
                and "desc_link" in self.row
            ):
                res: SearchResults = {
                    "link": self.row["link"],
                    "name": self.name,
    ...
    def threaded_search(self, page: int, what: str) -> None:
        ...
        parser = self.MyHtmlParser(self.url, self._seen_detail_links, self._detail_lock)
