# VERSION: 1.1
"""
SubsPlease (https://subsplease.org) search engine. Queries the subsite's JSON
API, one page at a time; file size is read from the xl parameter of each
...
class subsplease:
    url: str = "https://subsplease.org/"
    name: str = "SubsPlease"
    supported_categories: ClassVar[dict[str, str]] = {"all": ""}

    def search(self, what: str, _cat: str = "all") -> None:
        seen_magnets: set[str] = set()
        release_dates: dict[str, int] = {}
        for page in range(min(MAX_PAGES, 6)):
            search_url = f"https://subsplease.org/api/?f=search&tz=$&s={what}&p={page}"
            response = retrieve_url(search_url)
            try:
                response_value: object = cast(object, json.loads(response))
            except (TypeError, ValueError):
                break
            if not isinstance(response_value, dict) or not response_value:
                break
            response_json = cast(dict[str, object], cast(object, response_value))

            for result_name, raw_result_data in response_json.items():
                if not isinstance(raw_result_data, dict):
                    continue
                result_data = cast(dict[str, object], cast(object, raw_result_data))
                release_date_text = str(result_data.get("release_date", ""))
                if release_date_text not in release_dates:
                    try:
                        release_dates[release_date_text] = int(
                            datetime.strptime(
                                release_date_text, "%a, %d %b %Y %H:%M:%S %z"
                            ).timestamp()
                        )
                    except ValueError:
                        continue
                ...
                if not isinstance(downloads, list):
                    continue
                for raw_download in cast(list[object], cast(object, downloads)):
                    if not isinstance(raw_download, dict):
                        continue
                    download = cast(dict[str, object], cast(object, raw_download))
                    magnet_value = download.get("magnet")
                    if not isinstance(magnet_value, str) or not magnet_value:
                        continue
                    magnet_link = magnet_value
                    if magnet_link in seen_magnets:
                        continue
                    parsed_url = urlparse(magnet_link)
                    size_values = parse_qs(parsed_url.query).get("xl")
                    if not size_values:
                        continue
                    ...
                    res = SearchResults(
                        link=magnet_link,
                        name=f"[SubsPlease] {result_name} ({download.get('res', '')}p)",
                        size=size_values[0],
                        seeds=-1,
                        leech=-1,
                        engine_url=search_url,
                        desc_link="-1",
                        pub_date=release_dates[release_date_text],
                    )
                    _qbt_prettyPrinter(res)
                    if len(seen_magnets) >= MAX_DETAILS:
                        return
