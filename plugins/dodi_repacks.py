...
class dodi_repacks:
    url: str = "https://dodi-repacks.site/"
    name: str = "DODI Repacks"
    ...
    def search(self, what: str, _cat: str = "all") -> None:
        ...
        for raw_result in cast(list[object], cast(object, downloads)):
            ...
            if any(term in title.lower() for term in search_terms):
                timestamp = int(
                    datetime.strptime(str(result["uploadDate"]), "%Y-%m-%dT%H:%M:%S.%fZ")
                ...
                res = SearchResults(
                    ...
                    name=title,
                    size=cast(Union[float, int, str], result["fileSize"]),
                    seeds=-1,
                    leech=-1,
                    engine_url=self.url,
                    desc_link="-1",
    ...
    def download_link(self, result: Mapping[str, object]) -> str:
        ...
        if isinstance(uris, list) and uris:
            return str(cast(list[object], cast(object, uris))[0])
        return ""
