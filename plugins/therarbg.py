...
THERARB_MAGNET_HREF_RE = re.compile(r'href=["\']magnet:.+?["\']')
THERARB_SLUG_RE = re.compile(r"[ .]+")
...
class therarbg:
        ...
        def handle_starttag(self, tag: str, attrs: Sequence[tuple[str, str | None]]) -> None:
            ...
            if self.insideCell:
                if self.column == 2 and tag == self.A and not self.alreadyParseName:
                    ...
                    magnet_match = THERARB_MAGNET_HREF_RE.search(torrent_page)
                    if magnet_match is not None:
                        self.row["link"] = magnet_match.group().split('"')[1]
    ...
    def _result_from_json(self, post: dict[str, object]) -> SearchResults | None:
        ...
        slug = quote(THERARB_SLUG_RE.sub("-", name).lower(), safe="-_")
    ...
    def search(self, what: str, cat: str = "all") -> None:
        ...
        for _ in range(MAX_PAGES):
            ...
            if not isinstance(next_url, str) or not next_url or next_url in seen_pages:
                return
            page_url = next_url
