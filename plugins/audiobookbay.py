...
class audiobookbay:
        ...
        def handle_endtag(self, tag: str) -> None:
            if self.torrentReady:
                ...
                if not desc_link:
                    ...
                    return
                if (
                    self.details_fetched >= MAX_DETAILS
                    or desc_link in self.seen_detail_links
                ):
                    self.torrent_info = self.empty_torrent_info()
                    self.foundResult = False
                    self.torrentReady = False
                    return
                self.seen_detail_links.add(desc_link)
    ...
    def find_healthy_url(self) -> str | None:
        ...
        for url in self.urls[:MAX_DETAILS]:
    ...
    def search(self, what: str, cat: str = "all") -> str | None:
        ...
        try:
            ...
            total_pages = min(parser.totalPages, MAX_PAGES)
