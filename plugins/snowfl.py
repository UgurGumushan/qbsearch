...
class snowfl:
    ...
    class Parser:
        def __init__(self, url: str) -> None:
            self.url: str = url
            self.token: str = self.__retrieveToken()
        ...
        def __retrieveToken(self) -> str:
            ...
            file_name = cast(list[str], re.findall(r".+?\"(b.min.js\?.+)\"", index_html))[0]
            script = retrieve_url(self.url + file_name)
            # Retrieving the token
            token = cast(
                list[str],
                re.findall(r"\"([a-zA-Z0-9]+)\";\$\(\(function\(\){var e,t,n,r,o,a,i=", script),
            )[0]
            return token
    ...
    def search(self, what: str, _cat: str = "all") -> None:
        try:
            ...
            collection = cast(
                list[Torrent],
                cast(object, json.loads(retrieve_url(what))),
            )
            parser.feed(collection)
