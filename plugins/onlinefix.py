...
class onlinefix:
    ...
    def search(self, what: str, _cat: str = "all") -> None:
        ...
        try:
            response_json_value: object = cast(object, json.loads(response))
        except (TypeError, ValueError):
            return
        if not isinstance(response_json_value, dict):
            return
        response_json = cast(dict[str, object], cast(object, response_json_value))
        downloads = response_json.get("downloads")
        ...
        for raw_result in cast(list[object], cast(object, downloads))[:MAX_DETAILS]:
            ...
            if any(term in title.lower() for term in search_terms):
                download_link = self.download_link(result)
                if not download_link or download_link in seen_links:
                    continue
                seen_links.add(download_link)
                file_size = result.get("fileSize", -1)
                if not isinstance(file_size, (float, int, str)):
                    file_size = -1
                res = SearchResults(
                    link=download_link,
                    name=title,
                    size=cast(Union[float, int, str], file_size),
                    seeds=-1,
                    leech=-1,
                    engine_url=self.url,
                    desc_link="-1",
    ...
    def download_link(self, result: Mapping[str, object]) -> str:
        uris = result.get("uris")
        if isinstance(uris, list):
            for uri in cast(list[object], cast(object, uris)):
