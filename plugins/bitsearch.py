...
class bitsearch:
    ...
    def search(self, what: str, _cat: str = "all"):
        ...
        results_match = re.search(self.results_regex, retrievedHtml, re.MULTILINE)

        if results_match:
            m = re.search(r"\d+", results_match.group(0))
        ...
        if pages > 0:
            ...
            for page in range(2, min(pages, 10, MAX_PAGES) + 1):
                page_url = f"{self.url}/search?q={what}&page={page}"
                retrievedHtml = retrieve_url(page_url)
