...
class nyaa_phuong:
  ...
  supported_categories: ClassVar[dict[str, str]]  = {'all': '0', 'movies': '6', 'tv': '4', 'music': '1', 'games': '2', 'anime': '7', 'software': '3'}
  ...
  class RowParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if tag == 'table':
            self.depth += 1
            if not self.in_results and 'results' in params.get('class', ''):
  ...
  def search(self, what, cat='all'):
    ...
    number_of_page = math.ceil(float(total_results) / float(item_per_pages)) if item_per_pages != '0' else 1
