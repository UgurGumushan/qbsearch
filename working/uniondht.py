...
the tracker page in batches of 50 and prints rows from a worker thread that
drains a queue, so output is produced concurrently with parsing.
"""
...
class PrettyWorker(Thread):
    def __init__(self, queue: Queue[TorrentInfo]) -> None:
        super().__init__()
        self.queue = queue
    ...
    def run(self):
        while True:
            try:
                prettyPrinter(cast(SearchResults, cast(object, self.queue.get(timeout=2))))
...
class uniondht:
    ...
    class UnionDHTParser(HTMLParser):
        def __init__(self, url):
            ...
            self.print_queue: Queue[TorrentInfo] = Queue()
