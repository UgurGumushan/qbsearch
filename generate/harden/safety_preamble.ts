/** Canonical safety preamble copied into standalone plugin files. */
...
const BASE_PREAMBLE = PREAMBLE_LINES.join("\n");

const OVERRIDE_IMPORT = `    from typing import TYPE_CHECKING
    from typing import Callable as _QBTCallable`;
const OVERRIDE_SUPPORT = `if TYPE_CHECKING:
...
function addOverrideSupport(preamble: string): string {
  return preamble
    .replace("    from typing import Callable as _QBTCallable", OVERRIDE_IMPORT)
    .replace("\nHTTP_TIMEOUT = 20.0", `\n${OVERRIDE_SUPPORT}`);
}

export const SAFETY_PREAMBLE = BASE_PREAMBLE;
export const SAFETY_PREAMBLE_WITH_OVERRIDE = addOverrideSupport(BASE_PREAMBLE);
