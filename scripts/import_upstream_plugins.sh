...
# Maintainer-only provenance importer. It deliberately writes outside
# plugins/ so an upstream refresh cannot overwrite hardened installable files.
# Review and port changes into plugins/ explicitly.
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
UPSTREAM="${QBSEARCH_UPSTREAM_DIR:-$ROOT/external/upstream}"
mkdir -p "$UPSTREAM"
